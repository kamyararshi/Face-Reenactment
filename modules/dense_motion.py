...
class DenseMotionNetworkUpdater(nn.Module):
    """
    Module that predicting a dense motion from sparse motion representation given by kp_source and kp_driving
    """
    ...
    def __init__(self, block_expansion, num_blocks, max_features, num_kp, feature_channel, reshape_depth, compress,
                 estimate_occlusion_map=False):
        super(DenseMotionNetworkUpdater, self).__init__()
        # self.hourglass = Hourglass(block_expansion=block_expansion, in_features=(num_kp+1)*(feature_channel+1), max_features=max_features, num_blocks=num_blocks)
        self.hourglass = Hourglass(block_expansion=block_expansion, in_features=(num_kp+1)*(compress+3), max_features=max_features, num_blocks=num_blocks)

        # self.mask = nn.Conv3d(self.hourglass.out_filters, num_kp + 1, kernel_size=7, padding=3)
        self.update = nn.Conv3d(self.hourglass.out_filters, 3, kernel_size=7, padding=3)
    ...
    def forward(self, feature, kp_driving, kp_source):
        ...
        input = torch.cat([heatmap*sparse_motion.permute(0,1,5,2,3,4), deformed_feature], dim=2)
        input = input.view(bs, -1, d, h, w)
        ...
        update = self.update(prediction)
        
        # out_dict['update'] = update
        
        # deformation = sparse_motion + update
        deformation = update.permute(0, 2, 3, 4, 1)           # (bs, d, h, w, 3)
